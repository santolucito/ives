...
We place a constraint on the given example set that $\forall j. 0 \le j \le n. i_j \in \tau_i \land o_j \in \tau_o$. In practice this a trivial constraint, achieved by requiring the user to provide the types explicitly via type annotation (as we do), or using the compiler to infer types~\cite{ghc}, or by inferring the types based on regular expressions~\cite{gulwani_popl15} .

Let $T$ be the set of types (both inferred and explicitly declared) exposed to the top-level module from user code, imported libraries, and the core library. Additionally, we require that $\tau_i \in T \land
\tau_o \in T$.
...
However, using cycolmatic complexity verbatim means \codeinline{f} is more complex than \codeinline{g}.
...
In keeping with good functional programming style, $\mathcal{N}$(\codeinline{f = (1+)})=1/2 is considered more natural than $\mathcal{N}$(\codeinline{f x = 1 + x})=1/3.
This also favors predefined functions over generating new lambda terms (anonymous functions).
Taking the code from Listing \ref{natSyn}, $\mathcal{N}$(\codeinline{solution1}) has a naturalness score of 1/8, while $\mathcal{N}$(\codeinline{solution2}) has a score of 1/3.
